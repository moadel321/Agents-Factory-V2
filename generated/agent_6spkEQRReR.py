...
class TheQuestionIsAboutCommercialLicensingServicesAgent(BaseFlowAgent):
    ...
    async def go_Questions_outside_the_scope(self) -> Optional[Agent]:
        """سؤال خارج الاختصاص  — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
    ...
    async def go_go_back_to_welcome(self) -> Optional[Agent]:
        """المستخدم يريد الرجوع للبداية — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
    ...
    async def go_The_client_needs_to_file_a_report_about_the_issue_they_a_62dc(
        ...
        """المستخدم يحتاج الى رفع بلاغ بالمشكلة اللي تواجهه  — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
...
class WelcomeNodeAgent(BaseFlowAgent):
    ...
    async def go_The_client_has_finished_their_call_and_is_satisfied_with_56bf(
        ...
        """المستخدم اكتفى بالاجابة و راضي عنها  — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
    ...
    async def go_Any_question_regarding_the_Baladi_application(self) -> Optional[Agent]:
        """اي سؤال او استفسار عن تطبيق او منصة بلدي  — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
    ...
    async def go_For_inquiries_about_health_certificates(self) -> Optional[Agent]:
        """المستخدم طلب استفسار عن خدمات الشهادات الصحية  — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
    ...
    async def go_Inquiry_about_construction_permit_services(self) -> Optional[Agent]:
        """المستخدم طلب استفسار عن الرخص الانشائية — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
    ...
    async def go_The_question_is_about_commercial_licensing_services(
        ...
        """المستخدم طلب استفسار عن الرخص التجارية — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
...
class QuestionsOutsideTheScopeAgent(BaseFlowAgent):
    ...
    async def go_The_client_has_finished_their_call_and_is_satisfied_with_56bf(
        ...
        """اذا المستخدم انتهى من الحديث و كان راضي عن الاجابة ولا يحتاج الي شي اخر  — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
    ...
    @function_tool
    async def go_Any_question_regarding_the_Baladi_application(self) -> Optional[Agent]:
        """اهلين كيف اقدر اخدمك في تطبيق بلدي ؟ — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
    ...
    @function_tool
    async def go_Inquiry_about_construction_permit_services(self) -> Optional[Agent]:
        """السوال عن خدمات الرخص الانشائية  — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
    ...
    async def go_The_question_is_about_commercial_licensing_services(
        ...
        """السوال عن خدمات الرخص التجارية  — ملاحظة: استخدم أداة واحدة فقط في هذا الدور؛ لا تجمع أكثر من أداة."""
        flow_state: FlowState = self.session.userdata
        self._enable_preemptive_generation()
